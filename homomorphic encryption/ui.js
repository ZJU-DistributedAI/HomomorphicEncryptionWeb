...
export function predictResultCallback(callback){

  const predictResultButton=document.getElementById('predict_result');
  predictResultButton.addEventListener('click',()=>{
    predictResultButton.setAttribute('disabled',true);
    callback();
  });
}

export function getModelCallback(callback){

  const getModelButton=document.getElementById('get_model');
  getModelButton.addEventListener('click',()=>{
    getModelButton.setAttribute('disabled',true);
    callback();
  });
}
export function trainModelCallback(callback){

  const trainModelButton=document.getElementById('train_model');
  trainModelButton.addEventListener('click',()=>{
    trainModelButton.setAttribute('disabled',true);
    callback();
  });
}

export function initModelCallback(callback){

  const initModelButton=document.getElementById('init_model');
  initModelButton.addEventListener('click',()=>{
    initModelButton.setAttribute('disabled',true);
    callback();
  });
}

export function checkCurrentModelCallback(callback){

  const checkCurrentModelButton=document.getElementById('check_current_model');
  checkCurrentModelButton.addEventListener('click',()=>{
    checkCurrentModelButton.setAttribute('disabled',true);
    callback();
  });
}
